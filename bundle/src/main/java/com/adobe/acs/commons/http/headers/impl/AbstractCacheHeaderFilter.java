...
public abstract class AbstractCacheHeaderFilter implements Filter {
    ...
    static final class ServletRequestPredicates {
        ...
        protected final boolean allowAuthorized;

        ServletRequestPredicates(String[] patterns) {
            this(patterns, false, new String[] {}, new String[] {}, true);
        }

        ServletRequestPredicates(String[] patterns, boolean allowAllParameters, String[] disallowedParameterNames, String[] allowedParameterNames, boolean allowAuthorized) {
            ...
            this.allowAuthorized = allowAuthorized;
        }
    ...
    protected boolean accepts(final HttpServletRequest request) {
        ...
        if (StringUtils.equalsIgnoreCase("get", request.getMethod())) {
            if (requestPredicates.hasAllowedParameters(request)) {
                if (serverAgents.contains(DISPATCHER_AGENT_HEADER_VALUE)) {
                    if (requestPredicates.allowAuthorized || !isAuthorizedRequest(request)) {
                        return true;
                    } else {
