...
public class ResourceTypeBasedDispatcherMaxAgeHeaderFilter extends ResourceBasedDispatcherMaxAgeHeaderFilter {
    ...
    public ResourceTypeBasedDispatcherMaxAgeHeaderFilter(Config config, BundleContext bundleContext) {
        super(config.max_age(), new AbstractCacheHeaderFilter.ServletRequestPredicates(config.filter_pattern() ,config.allow_all_params(), config.block_params(), config.pass_through_params(), config.allow_authorized()), config.service_ranking(), bundleContext);
    }
