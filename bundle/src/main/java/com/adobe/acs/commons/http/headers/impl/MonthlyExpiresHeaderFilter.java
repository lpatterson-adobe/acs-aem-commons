...
public class MonthlyExpiresHeaderFilter extends AbstractExpiresHeaderFilter {
    ...
    public @interface Config {
        @AttributeDefinition(name = "Filter Patterns", description = "Restricts adding the headers to request paths which match any of the supplied patterns.", cardinality = Integer.MAX_VALUE, min = "1")
    ...
    public MonthlyExpiresHeaderFilter(Config config, BundleContext bundleContext) {
        super(config.expires_time(), new AbstractCacheHeaderFilter.ServletRequestPredicates(config.filter_pattern() ,config.allow_all_params(), config.block_params(), config.pass_through_params(), config.allow_authorized()), config.service_ranking(), bundleContext);
        this.config = config;
